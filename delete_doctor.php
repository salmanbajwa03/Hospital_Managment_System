<?php
	session_start();
	if(!isset($_SESSION['adminEmail'])){
		echo "<script>window.location.href='index.php';</script>";
	}
	else{
?>
<!DOCTYPE html>
<html>
<head>
	<title>Delete Doctor</title>

	<?php include('connection.php');?>


</head>

<body style="font-family:'Trebuchet MS', Helvetica, sans-serif;">

<?php include('admin_nav.php');?>
	
	<div class="container">
		<div class="jumbotron"><br><br>
		<form method="post">			
			<label>Doctor Email:-</label>
			<input type="text" class="form-control" name="uname" required><br>
		
			<button class="btn btn-info" name="submit_btn"> Submit</button>
		</form>
		</div>
	</div>

	<?php include('footer.php');?>
		
</body>
</html>
<?php
		if(isset($_POST['submit_btn'])){
			$doctor_deptname = $_POST['uname'];
			$sql="select * from doctor where doctor_email='$doctor_deptname'";
			$result = $conn->query($sql);
			$data = $result->fetch_assoc();
			$doc_id = $data['doctor_id'];	
			
			$sql1 = "delete from appointment where doctor_id='$doc_id'";
			$sql2 = "delete from doctor where doctor_id='$doc_id'";
		
			if($conn->query($sql1) === true && $conn->query($sql2) === true){
				echo "<script>alert('Doctor deleted Successfully');</script>";
			}
			else{
				echo "<script>alert('Error ');</script>";
			}
		}
	}
?>